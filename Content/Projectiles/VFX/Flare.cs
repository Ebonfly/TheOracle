...
public class Flare : ModProjectile
{
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        Texture2D ring = Assets.Extras.twirl_01.Value;
        float progress = Utils.GetLerpValue(20, 0, Projectile.timeLeft);
        for (int i = 0; i < 2; i++)
        {
            if (i == 0)
                Main.spriteBatch.Draw(tex, Projectile.Center - Main.screenPosition, null,
                Color.White with { A = 0 } * (Projectile.ai[2] == 0 ? 1 : Projectile.ai[2])* Projectile.scale*0.5f, Projectile.rotation, tex.Size() / 2f,
                progress * (Projectile.ai[0] == 0 ? 1 : Projectile.ai[0]),
                SpriteEffects.None, 0);
            Main.spriteBatch.Draw(tex, Projectile.Center - Main.screenPosition, null,
                Color.CornflowerBlue with { A = 0 } * (Projectile.ai[2] == 0 ? 1 : Projectile.ai[2])* Projectile.scale, Projectile.rotation+i*MathHelper.PiOver4, tex.Size() / 2f,
            ...
            switch ((int)Projectile.ai[1])
            {
                case 1:
                    Main.spriteBatch.Draw(ring, Projectile.Center - Main.screenPosition, null,
                        Color.CornflowerBlue with { A = 0 } * (Projectile.ai[2] == 0 ? 1 : Projectile.ai[2])* Projectile.scale, Projectile.rotation+MathHelper.Pi*i - progress*MathHelper.Pi, ring.Size() / 2f, 
        ...
        Main.spriteBatch.Draw(flare, Projectile.Center - Main.screenPosition, null,
            Color.White with { A = 0 } * (Projectile.ai[2] == 0 ? 1 : Projectile.ai[2])*MathF.Pow(Projectile.scale,4), Projectile.rotation, flare.Size() / 2f,
